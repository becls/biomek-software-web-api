# Deck Layout: Biomek i7 DualMultichannel

- Source: `deckLayouts.dualMultichannel.positions` in the Biomek Instrument Settings export.
...
10   [empty]   [WS1]     [TL1]     [P1]      [P6]      [P11]     [P16]     [P21]     [TL6]     [WS2]     [empty]
15   [TR1              ] [TL2]     [P2]      [P7]      [P12]     [P17]     [P22]     [TL7]     [TR2              ]
20   [empty]   [empty]   [TL3]     [P3]      [P8]      [P13]     [P18]     [P23]     [TL8]     [empty]   [empty]
25   [empty]   [empty]   [TL4]     [P4]      [P9]      [P14]     [P19]     [P24]     [TL9]     [empty]   [empty]
30   [empty]   [empty]   [TL5]     [P5]      [P10]     [P15]     [P20]     [P25]     [TL10]    [empty]   [empty]
```

## Deck (illustrative, resolved)

The block below mirrors the shape of `deckLayouts.dualMultichannel` in the export, with `$ref` pointers resolved and non-load-bearing keys removed. Values shown (coordinates in cm, `alpType`, labware `class` / `properties.liquidtype` / `volumeType`, device `name`, characteristic and required keys) are copied verbatim from the export; the surrounding `labware` object is reduced to those fields, and `device` is collapsed from the export's device object to just its `name` string.

```json
{
  "name": "DualMultichannel",
  "isDefault": false,
  "isReadOnly": false,
  "positions": {
    "p1": {
