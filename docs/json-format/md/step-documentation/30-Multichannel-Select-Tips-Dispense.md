# Multichannel Select Tips Dispense
...
The Select Tips Dispense step delivers liquid to a labware position using the
multichannel pod's currently loaded tip pattern. When `emptyTips` is `true`, it
dispenses all liquid currently in the tips (volume calculated from tip contents)
rather than using the `volume` parameter. After dispensing, it updates the
destination labware's liquid type only if it is not already set. Only the tips
that are physically loaded participate in the operation.

This step must be placed inside a `"Multichannel Select Tips"` container, after
tips have been loaded and liquid has been aspirated.

To perform a dispense operation using a **full** head of tips (96 for a 96
...
|-----|------|-------|----------|---------|-------------|
| `location` | `expression-capable string` | — | Yes | `""` | Deck position name or labware **instance** name of destination labware. Supports expressions (prefix with `=`). |
| `volume` | `expression-capable string` | µL | Conditional | `"0"` | Volume to dispense. Required when `emptyTips` is `false`. Must be non-negative. |
| `emptyTips` | `boolean` | — | No | `false` | If `true`, dispense all liquid in tips (ignores `volume`). |
...
| `liquidtype` | `expression-capable string` | — | Yes | `""` | Destination liquid classification. Commonly `"Tip Contents"` (auto-detect from tips) or a named liquid type. Must be non-empty. |
| `columnOffset` | `expression-capable string` | — | Conditional | `"1"` | Starting column (1-based) to place the back left loaded tip over. Required when destination labware is a microplate or tube rack. |
| `rowOffset` | `expression-capable string` | — | Conditional | `"1"` | Starting row (1-based) to place the back left loaded tip over. Required when destination labware is a microplate or tube rack. |
| `reservoirSection` | `expression-capable string` | — | Conditional | `"1"` | Reservoir section number to place the back left loaded tip over. Required when destination labware is a reservoir. |
...
| `overrideHeight` | `boolean` | — | No | `false` | If `true`, use custom height to dispense. |
| `height` | `number or expression-capable string` | mm | Conditional | — | Custom pipetting height. Required when `overrideHeight` is `true`. |
...
| `operation` | `string` | — | No | `"Dispense"` | Defaults to `"Dispense"`. Do not change; no other operations are valid. |
...
|-----------|---------------|-------|
| `operation` | `"Dispense"` | Defaults to `"Dispense"`; do not change. |
...
| `liquidtype` | Any named liquid, `"Tip Contents"` | `"Well Contents"` is **invalid** for dispense. |
...
4. When `emptyTips` is `false`, `volume` must be provided and non-negative — a negative volume raises a negative-volume error.
5. When `emptyTips` is `true`, volume is calculated from tip contents (non-air volumes summed).
6. Position must be reachable and contain labware.
7. Every active tip must map to a valid well, else a tip-miss error is raised. (Runtime motion collisions are a separate, later hardware check, not this enqueue validation.
8. When `overrideHeight` is `true`, `height` and `heightFrom` must be provided.
9. If `autoSelectPrototype` is `false`, then either `prototype` **must** name a valid technique or an inline `customPrototype` object **must** be bound. `customPrototype` takes precedence over both `autoSelectPrototype` and `prototype`.
...
- **Providing `volume` with `emptyTips: true`**: `volume` is ignored — actual amount comes from summed non-air tip contents.
- **Empty `labwareClass`**: The default is `""` (empty), which fails the non-empty check — supply a real class name.
- **Destination `liquidtype` unexpectedly unchanged**: The step only writes `liquidtype` to destination wells that don't already have one; existing labels are preserved.
